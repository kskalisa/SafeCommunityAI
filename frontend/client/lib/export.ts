...
export function downloadPdf(filename: string, title: string, sections: PdfSection[], meta: PdfMeta = {}) {
  const content = makePdf(title, sections, {
    ...
    subtitle: meta.subtitle ?? "Emergency incident records based on the applied report filters",
  });
...
function makePdf(title: string, sections: PdfSection[], meta: Required<PdfMeta>) {
  ...
  const centeredText = (value: string | number, center: number, baseline: number, size = 9, color = colors.ink, font = "F1") => {
    const raw = String(value ?? "");
    text(raw, center - estimateWidth(raw, size) / 2, baseline, size, color, font);
  };
  ...
  const startPage = () => {
    ...
    drawHeader();
    y = pageHeight - (wideTable ? 140 : 164);
  };

  const drawHeader = () => {
    const generated = new Date();
    const logoSize = wideTable ? 36 : 38;
    const logoTop = pageHeight - (wideTable ? 66 : 74);
    const metaWidth = wideTable ? 190 : 178;
    const metaX = pageWidth - margin - metaWidth;
    const metaStart = pageHeight - (wideTable ? 42 : 50);
    ...
    drawMetaPair("Generated At", generated.toLocaleString(), metaX, metaStart - 16, metaWidth);
    line(margin, pageHeight - (wideTable ? 116 : 132), pageWidth - margin, pageHeight - (wideTable ? 116 : 132), colors.blue);
  };
  ...
  const drawFilterSummary = () => {
    const filters = meta.filters.filter(Boolean).map((filter) => splitFilter(filter));
    if (filters.length === 0) return;

    const cellW = contentWidth / filters.length;
    ensureSpace(48);
    text("Applied Filters", margin, y, 8.5, colors.letter, "F2");
    rect(margin, y - 36, contentWidth, 26, "1 1 1");
    strokeRect(margin, y - 36, contentWidth, 26, colors.line);
    filters.forEach(([label, value], index) => {
      const x = margin + index * cellW;
      if (index > 0) line(x, y - 10, x, y - 36, "0.88 0.9 0.92");
      const summary = `${label}: ${value}`;
      text(truncate(summary, Math.max(10, Math.floor((cellW - 14) / 3.8))), x + 7, y - 26, 7.2, colors.ink, index === 0 ? "F2" : "F1");
    });
    y -= 50;
  };
  ...
  const drawSection = (section: PdfSection) => {
    ...
    ensureSpace(62);
    centeredText(section.title, pageWidth / 2, y, 11, colors.ink, "F2");
    y -= 15;
    centeredText("Filtered incident records generated from the selected criteria.", pageWidth / 2, y, 8, colors.letter);
    y -= 18;
    if (!filtersRendered) {
      drawFilterSummary();
    ...
    text("Incident Details", margin, y, 9, colors.ink, "F2");
    y -= 16;
...
function columnWidths(headers: string[], availableWidth: number) {
  ...
  const preset = normalized.map((header) => {
    ...
    if (header.includes("incident type")) return 1.05;
    if (header.includes("priority")) return 0.85;
    if (header.includes("status")) return 0.9;
