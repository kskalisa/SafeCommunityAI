...
import { BadgeCheck, Download, FileText, LifeBuoy, Radio, ShieldCheck } from "lucide-react";
...
import type { ResponderDetailResponse, ResponderStatus } from "@/types/api";
  ...
  return (
    <AdminPageShell title="Responder Management" description="Track responder readiness, verification, assignments, and field availability." actions={<AdminButton variant="secondary" onClick={exportRows}><Download className="h-4 w-4" />Export responders</AdminButton>}>
      ...
      <div className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[1fr_13rem_13rem]">
        <SearchInput value={search} onChange={setSearch} placeholder="Search responders or resources..." />
      ...
      <Panel title="Responder Directory" description={`${rows.length} matching responders`}>
        <div className="grid gap-4 lg:grid-cols-2">
          {rows.map((responder) => <ResponderCard key={responder.id} responder={responder} />)}
...
function ResponderCard({ responder }: { responder: ResponderDetailResponse }) {
  const downloadCertificate = async () => {
    const blob = await usersApi.downloadResponderCertificate(responder.id);
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = responder.certificateFileName || `certificate-${responder.id}`;
  ...
  return (
    <div className="rounded-lg border border-slate-200 p-4 transition hover:border-red-200">
      ...
      <div className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><p>Organization: <b>{responder.organization || "Not set"}</b></p><p>Resources: <b>{responder.resources?.length ? responder.resources.map((resource) => resource.name).join(", ") : "Not set"}</b></p><p>Certificate: <b>{responder.certificateFileName || "Not set"}</b></p><p>Account: <b>{responder.enabled ? "Active" : "Disabled"}</b></p></div>
      {responder.certificateUrl ? <button type="button" onClick={downloadCertificate} className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-red-700 hover:text-red-800"><FileText className="h-4 w-4" />Download certificate</button> : null}
