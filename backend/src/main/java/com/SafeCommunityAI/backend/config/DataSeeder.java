...
public class DataSeeder {
    @Bean
    CommandLineRunner seed(UserRepository users, ResourceRepository resources, ResponderProfileRepository responderProfiles, HospitalRepository hospitals, AuthService authService, IncidentService incidentService, PasswordEncoder passwordEncoder, AuditService auditService) {
        return args -> {
            ...
            authService.register(new RegisterRequest("Demo Citizen", "citizen@demo.com", "demo123", Role.CITIZEN, "+250700000001", true, null));
            authService.register(new RegisterRequest("Demo Responder", "responder@demo.com", "demo123", Role.RESPONDER, "+250700000002", true, "Kigali EMS"));
            authService.register(new RegisterRequest("Demo Dispatcher", "dispatcher@demo.com", "demo123", Role.DISPATCHER, "+250700000003", true, null));
            Resource ambulance = resources.save(Resource.builder().name("Ambulance 12").type("AMBULANCE").status(ResourceStatus.AVAILABLE).location("Nyarugenge").build());
            resources.save(Resource.builder().name("Fire Truck 4").type("FIRE_TRUCK").status(ResourceStatus.AVAILABLE).location("Kacyiru").build());
            users.findByEmail("responder@demo.com").flatMap(responderProfiles::findByUser).ifPresent(profile -> {
                profile.getResources().add(ambulance);
                profile.setAvailabilityStatus(com.SafeCommunityAI.backend.enums.ResponderStatus.AVAILABLE);
                responderProfiles.save(profile);
            });
